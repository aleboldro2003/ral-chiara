 ...
 *
 * E' l'unico punto in cui il motore incontra un file JSON. Aggiungere l'anno
 * 2027 significa creare `parametri-2027.json` e aggiungere una riga qui: nessun
 * modulo di calcolo va toccato.
 *
 * Il cast a `ParametriAnno` e' l'unico del progetto e serve a restringere i tipi
 * larghi che TypeScript deriva da un import JSON (`string` invece delle union
 * letterali come "progressivaPerScaglioni"). Non e' una verifica: la validazione
 * strutturale a runtime, che e' cio' che protegge davvero da un file di parametri
 * malformato, vive nel motore ed e' agganciata qui sotto.
 */
